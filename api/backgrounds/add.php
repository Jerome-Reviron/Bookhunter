...
try {
    $stmt = $pdo->prepare("
        INSERT INTO backgrounds (id, label, css_class, image_url, is_premium, price_points)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$id, $label, $cssClass, $imageUrl, $isPremium, $pricePoints]);
